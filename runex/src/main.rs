...
/// Re-export the leaf util fns under main's namespace so existing
/// in-file call sites keep compiling without churn. New code should
/// `use crate::util::…` directly. Items that the production main
/// path doesn't reference (test-only constants, the
/// `prompt_confirm_from` inner helper used by unit tests) are
/// re-exported behind `#[cfg(test)]` to keep the production-build
/// `dead_code` lint clean.
use util::path::{make_command_exists, make_command_exists_owned};
use util::prompt::{prompt_confirm, read_rc_content};
use util::shell::{detect_shell, resolve_shell};
