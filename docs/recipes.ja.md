...
貼り付けたら `runex config reload` を実行し、新しいシェルを開いてください。多くのシェルでは、フックがキー押下のたびに設定を読むので、貼り付けた `[[abbr]]` は次のキー押下から効きます。ただし Git Bash は略語の表をシェル連携に埋め込み、`[keybind]`（レシピ 5、6、6b）はどのシェルでも埋め込まれるので、reload がどこでも成り立つ規則です。
